...
<head>
    ...
    <title>ACSM FITT-VP 智能環狀運動處方系統 (Beta v1.0)</title>
...
<div class="app-container">
    ...
    <aside class="sidebar">
        <div class="app-title">
            <h1>ACSM FITT-VP<br>智能環狀運動處方</h1>
            <p style="color: #666;">System Beta v1.0</p>
        </div>

        <div class="profile-group">
            <div class="profile-item"><label class="profile-label">使用者編號</label><input type="text" id="u-id" class="profile-input" value="U-001" placeholder="ID"></div>
            <div class="profile-item"><label class="profile-label">性別</label><select id="u-gen" class="profile-select"><option>女</option><option>男</option></select></div>
            <div class="profile-item"><label class="profile-label">年齡</label><input type="number" id="u-age" class="profile-input" value="60"></div>
            <div class="profile-item"><label class="profile-label">身高 (cm)</label><input type="number" id="u-height" class="profile-input" value="160" oninput="calcBMI()"></div>
            <div class="profile-item"><label class="profile-label">體重 (kg)</label><input type="number" id="u-weight" class="profile-input" value="55" oninput="calcBMI();updTotals()"></div>
            
            <div class="bmi-box">
                <div><span style="font-size:0.8rem; color:#777;">BMI</span><br><span id="bmi-val" class="bmi-val">21.5</span></div>
                <span id="bmi-tag" class="bmi-tag" style="background:var(--bmi-normal)">正常</span>
            </div>
        </div>

        <div class="stats-container">
            <div class="stat-card"><span class="stat-label">累積時間</span><span class="stat-value"><span id="t-time">0</span> <small>min</small></span></div>
            <div class="stat-card"><span class="stat-label">總 METs</span><span class="stat-value" id="t-mets">0</span></div>
            <div class="stat-card"><span class="stat-label">熱量消耗</span><span class="stat-value"><span id="t-kcal">0</span> <small>Kcal</small></span></div>
        </div>
    </aside>
    ...
    <main class="main-content">
        ...
        <div class="ai-section">
            <div class="ai-header"><h3>🤖 AI 運動處方規劃 (Clinical Evidence)</h3></div>
            <div class="ai-form-grid">
                <div><label class="profile-label">目標族群</label><select id="ai-pop" class="profile-select"><option value="healthy">一般健康 (Healthy)</option><option value="elderly">高齡/虛弱 (Elderly)</option><option value="dynapenia">力弱症 (Dynapenia)</option><option value="hypertension">高血壓 (Hypertension)</option><option value="metabolic">代謝症候群 (Metabolic)</option></select></div>
                <div><label class="profile-label">訓練目標</label><select id="ai-goal" class="profile-select"><option value="maintain">健康維持</option><option value="weight">減重燃脂</option><option value="cardio">心肺提升</option><option value="muscle">增肌強化</option><option value="bp">血壓控制</option></select></div>
                <div><label class="profile-label">膝關節狀況</label><select id="ai-knee" class="profile-select"><option value="no">正常</option><option value="yes">疼痛 / 退化</option></select></div>
                <div><label class="profile-label">呼吸道風險</label><select id="ai-asthma" class="profile-select"><option value="no">無風險</option><option value="yes">氣喘誘發</option></select></div>
            </div>
            <button class="btn-analyze" onclick="runAI()">⚡ 開始分析並產生處方</button>

            <div id="ai-result" class="ai-result-container">
                <div style="background:#f3e5f5; padding:12px 20px; font-weight:700; color:#6a1b9a; border-bottom:1px solid #e1bee7;">📊 處方設計摘要</div>
                <div style="padding:20px; background:#fff;">
                    <div class="analysis-badges" id="ai-badges"></div>
                    <div id="ai-text" style="color:#333; line-height:1.6; margin-bottom:10px;"></div>
                    <div class="sequence-list"><div style="font-weight:700; margin-bottom:5px; color:#7c4dff;">🔄 建議訓練順序 (單站 5 分鐘):</div><div id="ai-sequence"></div></div>
                </div>
            </div>
        </div>
        ...
        <div class="grid-container">
            <div class="equip-card" id="card-rec">
                <span class="highlight-tag">SEQ 1</span><div class="card-header"><span>臥式腳踏車</span><button class="btn-icon" onclick="info('rec')">?</button></div>
                <div class="card-body">
                    <div><div class="control-row"><span>時間(min)</span><span class="val-display" id="v-t-rec">5</span></div><input type="range" id="i-t-rec" min="0" max="30" step="5" value="5" oninput="upd('rec')"></div>
                    <div><div class="control-row"><span>阻力(Lv)</span><span class="val-display" id="v-r-rec">8</span></div><input type="range" id="i-r-rec" min="0" max="40" value="8" oninput="upd('rec')"></div>
                    <div class="warning-msg" id="w-rec">⚠️ 阻力過高</div><div><div class="preview-m" id="m-rec">--</div><span class="preview-label">METs-min</span></div><button class="btn-add" onclick="add('rec')">+ 加入</button>
                </div>
            </div>
            <div class="equip-card" id="card-up">
                <span class="highlight-tag">SEQ 2</span><div class="card-header"><span>立式腳踏車</span><button class="btn-icon" onclick="info('up')">?</button></div>
                <div class="card-body">
                    <div><div class="control-row"><span>時間(min)</span><span class="val-display" id="v-t-up">5</span></div><input type="range" id="i-t-up" min="0" max="30" step="5" value="5" oninput="upd('up')"></div>
                    <div><div class="control-row"><span>阻力(Lv)</span><span class="val-display" id="v-r-up">10</span></div><input type="range" id="i-r-up" min="0" max="40" value="10" oninput="upd('up')"></div>
                    <div><div class="preview-m" id="m-up">--</div><span class="preview-label">METs-min</span></div><button class="btn-add" onclick="add('up')">+ 加入</button>
                </div>
            </div>
            <div class="equip-card" id="card-run">
                <span class="highlight-tag">SEQ 3</span><div class="card-header"><span>跑步機</span><button class="btn-icon" onclick="info('run')">?</button></div>
                <div class="card-body">
                    <div><div class="control-row"><span>時間(min)</span><span class="val-display" id="v-t-run">5</span></div><input type="range" id="i-t-run" min="0" max="30" step="5" value="5" oninput="upd('run')"></div>
                    <div><div class="control-row"><span>速度(kph)</span><span class="val-display" id="v-s-run">5.0</span></div><input type="range" id="i-s-run" min="1" max="18" step="0.1" value="5" oninput="upd('run')"></div>
                    <div class="warning-msg" id="w-run">⚠️ 速度快!</div><div><div class="preview-m" id="m-run">--</div><span class="preview-label">METs-min</span></div><button class="btn-add" onclick="add('run')">+ 加入</button>
                </div>
            </div>
            <div class="equip-card" id="card-row">
                <span class="highlight-tag">SEQ 4</span><div class="card-header"><span>划船機</span><button class="btn-icon" onclick="info('row')">?</button></div>
                <div class="card-body">
                    <div><div class="control-row"><span>時間(min)</span><span class="val-display" id="v-t-row">5</span></div><input type="range" id="i-t-row" min="0" max="30" step="5" value="5" oninput="upd('row')"></div>
                    <div><div class="control-row"><span>阻力(Lv)</span><span class="val-display" id="v-r-row">5</span></div><input type="range" id="i-r-row" min="1" max="10" value="5" oninput="upd('row')"></div>
                    <div class="warning-msg" id="w-row">⚠️ 勿閉氣</div><div><div class="preview-m" id="m-row">--</div><span class="preview-label">METs-min</span></div><button class="btn-add" onclick="add('row')">+ 加入</button>
                </div>
            </div>
            <div class="equip-card" id="card-ellip">
                <span class="highlight-tag">SEQ 5</span><div class="card-header"><span>橢圓機</span><button class="btn-icon" onclick="info('ellip')">?</button></div>
                <div class="card-body">
                    <div><div class="control-row"><span>時間(min)</span><span class="val-display" id="v-t-ellip">5</span></div><input type="range" id="i-t-ellip" min="0" max="30" step="5" value="5" oninput="upd('ellip')"></div>
                    <div><div class="control-row"><span>強度</span><span class="val-display" id="v-r-ellip">中</span></div><input type="range" id="i-r-ellip" min="1" max="3" step="1" value="2" oninput="upd('ellip')"></div>
                    <div><div class="preview-m" id="m-ellip">--</div><span class="preview-label">METs-min</span></div><button class="btn-add" onclick="add('ellip')">+ 加入</button>
                </div>
            </div>
        </div>
        ...
        <div class="summary-panel">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                <h3 style="margin:0; font-size:1.3rem; color:var(--primary-dark);">📋 處方清單</h3>
                <div><button class="btn-custom" onclick="openCustomModal()">➕ 自定義</button></div>
            </div>
            <table id="lst"><thead><tr><th>順序</th><th>器材名稱</th><th>設定參數</th><th>時間</th><th>M值</th><th>操作</th></tr></thead><tbody></tbody></table>
            <div style="text-align:right; margin-top:15px;">
                <button class="btn-clear" onclick="clearPlan()">🗑️ 清空</button>
                <button class="btn-save-cal" onclick="saveCal()">💾 儲存至月曆</button>
                <button class="btn-export" onclick="exportDoc()">📄 匯出 Word 報表</button>
            </div>
        </div>

        <div class="chart-section">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                <h3 style="margin:0; color:var(--primary-dark);">📈 每週 METs 累積趨勢</h3>
            </div>
        ...
        <div class="calendar-section">
            <div class="calendar-header"><button class="cal-btn" onclick="mv(-1)">◀</button><div class="cal-title" id="cal-t"></div><button class="cal-btn" onclick="mv(1)">▶</button></div>
...
<div id="mod" class="modal" onclick="this.style.display='none'">
    <div class="modal-content" onclick="event.stopPropagation()">
        <div class="modal-header">
            <h2 class="modal-title" id="mod-t">設備名稱</h2>
...
<div id="edit-mod" class="modal" onclick="closeEditModal()">
    <div class="modal-content" onclick="event.stopPropagation()">
        <h3 id="edit-title" style="margin-top:0; color:var(--primary-dark)">編輯項目</h3>
        <div style="display:flex; flex-direction:column; gap:10px;">
            <input type="hidden" id="edit-idx">
            <div><label style="font-weight:600; font-size:0.9rem;">器材名稱</label><input type="text" id="edit-name" class="profile-input"></div>
            <div><label style="font-weight:600; font-size:0.9rem;">設定參數</label><input type="text" id="edit-desc" class="profile-input"></div>
            <div><label style="font-weight:600; font-size:0.9rem;">運動時間 (min)</label><input type="number" id="edit-time" class="profile-input"></div>
            <div><label style="font-weight:600; font-size:0.9rem;">M 值</label><input type="number" id="edit-m" class="profile-input"></div>
            <div style="text-align:right; margin-top:10px;">
                <button class="cal-btn" onclick="closeEditModal()" style="margin-right:10px;">取消</button>
                <button class="btn-save-cal" onclick="saveEdit()">確認儲存</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    const db = {
        'rec': { 
            t: "臥式腳踏車 (Recumbent Bike)", 
            desc: "器材說明：座位設有靠背，雙腳向前踩踏。相較於立式單車，此設計能顯著降低腰椎壓力，並提供更穩定的支撐，減少對核心肌群的依賴。<br><br>作用肌群：主要針對股四頭肌（大腿前側）、臀大肌與小腿肌群。膕旁肌（大腿後側）為輔助。<br><br>注意事項：<br>1. 座椅前後位置應調整至踩踏到底時，膝蓋保持微彎（約 10-15 度），避免膝蓋鎖死。<br>2. 上半身緊貼靠背，避免過度前傾或後仰。<br>3. 適合高齡者、肥胖者、背痛患者或平衡感較差者。" 
        },
        'up': { 
            t: "立式腳踏車 (Upright Bike)", 
            desc: "器材說明：模擬戶外騎乘姿勢，身體重心較高，需要核心肌群參與以維持平衡。提供標準的有氧訓練效果。<br><br>作用肌群：股四頭肌、膕旁肌、小腿肌群（腓腸肌、比目魚肌）。核心肌群需收縮以穩定軀幹。<br><br>注意事項：<br>1. 座墊高度應調整至與髖骨同高。<br>2. 踩踏到底時膝蓋微彎，不可完全伸直。<br>3. 避免將身體重量完全壓在手把上，應保持手肘微彎放鬆。<br>4. 適合一般健康族群及心肺耐力訓練。" 
        },
        'run': { 
            t: "跑步機 (Treadmill)", 
            desc: "器材說明：提供步行、快走或跑步的全身性負重運動平台。是燃燒熱量效率最高的有氧器材之一，能有效增強骨質密度。<br><br>作用肌群：全身性運動，主要強化股四頭肌、膕旁肌、臀肌、小腿肌群及核心肌群。<br><br>注意事項：<br>1. 務必夾上安全夾，以防跌倒時機器能自動緊急停止。<br>2. 雙眼直視前方，不要低頭看腳或螢幕過久。<br>3. 避免過度握緊扶手，應讓雙臂自然擺動。<br>4. 膝蓋或腳踝有傷者應評估後使用。" 
        },
        'row': { 
            t: "划船機 (Rowing Machine)", 
            desc: "器材說明：模擬水上划船動作，屬於全身性的綜合訓練器材，能同時訓練心肺耐力與肌力，特別是背部肌群。<br><br>作用肌群：涵蓋全身約 80% 肌群，包括背闊肌、菱形肌、二頭肌、股四頭肌、臀大肌及核心肌群。<br><br>注意事項：<br>1. 動作順序：腿蹬 -> 軀幹後傾 -> 手拉；回程相反。<br>2. 保持背部挺直，不可駝背。<br>3. 高血壓患者需注意呼吸調節，避免閉氣用力 (Valsalva maneuver)，以免血壓瞬間飆升。" 
        },
        'ellip': { 
            t: "橢圓機 (Elliptical Trainer)", 
            desc: "器材說明：結合踏步機、滑雪機與單車的特點，提供橢圓形的運動軌跡。特點是雙腳不離開踏板，對關節衝擊力（Impact）極低。<br><br>作用肌群：股四頭肌、膕旁肌、臀肌。若配合手把推拉，亦可訓練胸肌、背肌與三頭肌。<br><br>注意事項：<br>1. 腳掌應平貼踏板，避免腳跟懸空造成小腿緊繃。<br>2. 保持身體直立，不要過度前傾。<br>3. 適合膝關節退化、體重過重或受傷復健期的使用者。" 
        }
    };
    ...
    function calcBMI(){
        ...
        if(h>0 && w>0){
            ...
            let t="正常", c="var(--bmi-normal)";
            if(bmi<18.5){t="過輕";c="var(--bmi-under)"}
            else if(bmi>=24 && bmi<27){t="過重";c="var(--bmi-over)"}
            else if(bmi>=27 && bmi<30){t="輕肥";c="var(--bmi-obese)"}
            else if(bmi>=30){t="中重肥";c="var(--bmi-obese)"}
    ...
    function upd(k){
        ...
        if(k==='ellip'){ let r=parseInt(document.getElementById(`i-r-${k}`).value); document.getElementById(`v-r-${k}`).innerText=(r==1?"慢":r==2?"中":"快"); m=Math.round(t*(r==1?4:r==2?6:9)); }
    ...
    function rend(){
        ...
        plan.forEach((x,i)=>{ 
            ...
            b.innerHTML+=`<tr><td>${i+1}</td><td>${x.name}</td><td>${x.d}</td><td>${x.t} min</td><td>${x.m}</td><td><button class="cal-btn" onclick="openEdit(${i})" style="padding:4px 8px;margin-right:5px">✎</button><button class="cal-btn" onclick="del(${i})" style="padding:4px 8px;color:red">X</button></td></tr>`;
        });
    ...
    function clearPlan(){ if(confirm("清空?")){plan=[];rend();} }
    function updTotals(){
    ...
    function openCustomModal(){
        isEditing=false; document.getElementById('edit-title').innerText="新增項目";
    ...
    function openEdit(idx){
        ...
        document.getElementById('edit-title').innerText="編輯項目"; document.getElementById('edit-idx').value=idx;
    ...
    function runAI(){
        ...
        let st=5, unit=5, r_t="";
        if(pop==='elderly'||pop==='dynapenia'||knee==='yes'){ st=3; r_t="高風險/疼痛：設定最低有效劑量 (15分鐘)。"; } 
        else if(goal==='weight'||pop==='metabolic'){ st=6; r_t="減重/代謝：設定最大建議時間 (30分鐘)。"; }
        else { st=5; r_t="一般標準：設定 (25分鐘)。"; }
        ...
        document.getElementById('ai-text').innerHTML=`
            <div><strong>針對：</strong>${pTxt} / ${gTxt}</div>
            <div style="margin-top:5px;color:#004d40"><strong>💡 策略：</strong>${r_t} 為確保交叉訓練，強制組合 ${uniq} 種不同器材。</div>
        `;
    ...
    function saveCal(){
        ...
        localStorage.setItem('fittvp_cal_v10',JSON.stringify(cData)); drawCal(cy,cm); alert("已儲存");
    }
    function delCal(k){ if(confirm(`刪除 ${k}?`)){ delete cData[k]; localStorage.setItem('fittvp_cal_v10',JSON.stringify(cData)); drawCal(cy,cm); } }
    ...
    function exportDoc() {
        if(plan.length===0){alert("請先建立處方");return;}
        ...
        let strategy=document.getElementById('ai-text').innerText || "手動規劃";
        ...
        let html = `
            ...
            <head><meta charset='utf-8'><title>運動處方報告</title>
            <style>
                ...
                <h1>ACSM FITT-VP 專業運動處方報告</h1>
                <p style="text-align:right;">報告日期：${d} | ID：${id}</p>
                
                <h2>1. 個人身體評估 (Assessment)</h2>
                <table style="border:none;">
                    <tr><td>性別：${gender}</td><td>年齡：${age} 歲</td><td>身高：${h} cm</td></tr>
                    <tr><td>體重：${w} kg</td><td>BMI：${bmi}</td><td></td></tr>
                </table>

                <h2>2. 臨床決策分析 (Analysis)</h2>
                <ul>
                    <li><strong>目標族群：</strong> ${pop}</li>
                    <li><strong>訓練目標：</strong> ${goal}</li>
                    <li><strong>處方策略：</strong> ${strategy}</li>
                </ul>

                <h2>3. 運動處方內容 (Prescription)</h2>
                <table>
                    <thead><tr><th>順序</th><th>器材設備</th><th>強度設定</th><th>時間</th><th>負荷 (METs)</th></tr></thead>
                    <tbody>${rows}</tbody>
                </table>

                <h2>4. 預估成效 (Outcome Projection)</h2>
                <div class="stat-box">
                    總時間：${tT} 分鐘  |  總運動負荷：${tM} METs  |  預估消耗熱量：${tK} Kcal
                </div>

                <br><br>
                <div style="border-top:1px solid #999; margin-top:30px; padding-top:10px; font-size:0.8rem; text-align:center;">
                    本報告由 ACSM FITT-VP 智能系統 Beta v1.0 生成，僅供參考，實際執行請諮詢專業教練或醫師。
                </div>
            </body></html>`;
